...
export function useGraphActions() {
  ...
  const sendWebSocketMessage = useCallback((type: string, payload: Record<string, any>, skipWorkflowCheck = false) => {
    if (!selectedWorkflow && !skipWorkflowCheck) return;
    const workflowId = skipWorkflowCheck ? payload.workflowId : selectedWorkflow;
    sendMessage({ type, workflow_id: workflowId, ...payload });
  ...
  return {
    ...
    onCreateWorkflow: useCallback((workflowId: string) => {
      sendWebSocketMessage('create_workflow', { workflowId: workflowId }, true);
    }, [sendWebSocketMessage]),
  }
}
