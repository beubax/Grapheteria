...
// Function to parse run ID into a readable timestamp
const parseRunIdToTimestamp = (runId: string): string => {
  try {
    // Extract date and time parts: YYYYMMDD_HHMMSS_mmm
    const year = runId.substring(0, 4);
    const month = runId.substring(4, 6);
    const day = runId.substring(6, 8);
    const hour = runId.substring(9, 11);
    const minute = runId.substring(11, 13);
    const second = runId.substring(13, 15);
            ...
            {workflowLogs.map((runId) => (
              ...
              >
                <div className="flex justify-between items-center">
                  <span>{runId}</span>
                  <span className="text-sm text-gray-500">{parseRunIdToTimestamp(runId)}</span>
                </div>
              </div>
            ))}
          </div>
             ...
             runLogs.forked_from && (
              <div className="text-sm text-black-500">
                ...
                <span 
                  ...
                  onClick={() => navigate(`/logs/${workflowId}/${runLogs.forked_from}`)}
                >
                  {runLogs.forked_from}
                </span>
                 {' '}at step {runLogs.forked_step}
              </div>
            )}
          </div>
        </div>
