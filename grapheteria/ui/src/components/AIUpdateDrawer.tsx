...
import { Sparkles, X, Bot, User, Plug, Loader2 } from 'lucide-react';
...
const AIUpdateDrawer: React.FC<AIUpdateDrawerProps> = ({ open, onOpenChange, workflowId }) => {
  ...
  // Get the current workflow and its selected integrations
  const currentWorkflow = workflowId ? workflows[workflowId] : undefined;
  const initialSelectedIntegrations = currentWorkflow?.tools || [];
  const [selectedIntegrations, setSelectedIntegrations] = useState<string[]>(initialSelectedIntegrations);

  // Keep local state in sync with workflow changes
  useEffect(() => {
    setSelectedIntegrations(initialSelectedIntegrations);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [workflowId, currentWorkflow?.tools?.length]);

  // Handler to select/deselect integrations for the workflow
  const handleToggleIntegration = (integrationId: string) => {
    let newSelected: string[];
    if (selectedIntegrations.includes(integrationId)) {
      newSelected = selectedIntegrations.filter(id => id !== integrationId);
    } else {
      newSelected = [...selectedIntegrations, integrationId];
    }
    setSelectedIntegrations(newSelected);
    // If you want to update the backend/store, do it here (e.g., call onUpdateWorkflow or setWorkflows)
  };
  ...
  const handleSend = async () => {
    ...
    try {
      const { data, error } = await updateWorkflow(workflowId, input, selectedIntegrations);
      if (error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex border-b bg-white">
        <button
          className={`flex-1 py-2 text-center font-medium transition border-b-2 ${activeTab === 'chat' ? 'border-purple-600 text-purple-700 bg-purple-50' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
          onClick={() => setActiveTab('chat')}
        >
          <Bot className="inline-block mr-1 h-4 w-4" /> Chat
        </button>
        <button
          className={`flex-1 py-2 text-center font-medium transition border-b-2 ${activeTab === 'integrations' ? 'border-purple-600 text-purple-700 bg-purple-50' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}
          onClick={() => setActiveTab('integrations')}
        >
          <Plug className="inline-block mr-1 h-4 w-4" /> Integrations
        </button>
      </div>
      {/* Content */}
      <div className="flex-1 overflow-hidden relative" style={{ pointerEvents: 'auto' }}>
        {activeTab === 'chat' && (
          <div className="flex flex-col h-full">
            ...
            <div className="border-t bg-white p-3 flex h-[60px]" style={{ position: 'relative', zIndex: 50 }}>
              <input
