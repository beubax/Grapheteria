...
class InboundHandler:
    ...
    async def handle_client_message(server, websocket, message_data):
        ...
        if message_type.startswith('log_'):
            await InboundHandler._handle_log_message(server, websocket, message_data)
        ...
        if message_type == 'step':
            ...
            workflow_path = server.workflow_paths[workflow_id]
    ...
    @staticmethod
    async def _handle_log_message(server, websocket, data):
        ...
        match data['type']:
            case 'log_start_debug_session':
                ...
                print(f"Debug sessions: {server.debug_sessions}")
                # Store which run this client is debugging
                workflow_id = data.get('workflow_id')
                run_id = data.get('run_id')
                
                if not workflow_id or not run_id or workflow_id not in server.workflows:
                    return
                    
                server.debug_sessions[websocket] = {
            ...
            case 'log_end_debug_session':
                ...
                print(f"Debug sessions: {server.debug_sessions}")
                if websocket in server.debug_sessions:
                    del server.debug_sessions[websocket]
